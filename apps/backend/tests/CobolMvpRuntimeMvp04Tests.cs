...
public class CobolMvpRuntimeMvp04Tests
{
    [Theory]
    [InlineData("input1.txt", "expected1.txt")]
    [InlineData("input2.txt", "expected2.txt")]
    public void ProcessFile_Mvp04Samples_MatchesExpectedOutput(string inputFile, string expectedFile)
    {
        string backendRoot = GetBackendRoot();
        string repoRoot = Path.GetFullPath(Path.Combine(backendRoot, "..", ".."));

        string inPath = Path.Combine(repoRoot, "samples", "data", "mvp04", inputFile);
        string expectedPath = Path.Combine(repoRoot, "samples", "data", "mvp04", expectedFile);
        string outPath = Path.Combine(Path.GetTempPath(), "mvp04_test_out_" + Guid.NewGuid().ToString("N") + ".txt");

        try
        {
            Mvp04Program.ProcessFile(inPath, outPath);
