...
public class CobolMvpRuntimeMvp10Tests
{
    [Theory]
    [InlineData("input1.txt", "expected1.txt")]
    [InlineData("input2.txt", "expected2.txt")]
    public void ProcessFile_Mvp10Samples_MatchesExpectedOutput(string inputFile, string expectedFile)
    {
        string backendRoot = GetBackendRoot();
        string repoRoot = Path.GetFullPath(Path.Combine(backendRoot, "..", ".."));

        string inPath = Path.Combine(repoRoot, "samples", "data", "mvp10", inputFile);
        string expectedPath = Path.Combine(repoRoot, "samples", "data", "mvp10", expectedFile);
        string outPath = Path.Combine(Path.GetTempPath(), "mvp10_test_out_" + Guid.NewGuid().ToString("N") + ".txt");

        try
        {
            Mvp10Program.ProcessFile(inPath, outPath);
